...
namespace Decorator {
	...
	public class DeserializationGrunt {
		...
		static DeserializationGrunt() {
			var methods = typeof(DeserializationGrunt)
							.GetMethods();

			foreach (var i in methods) {
				if (i.IsGenericMethodDefinition)
					foreach (var j in i.MakeGenericMethod(typeof(object))
										.GetParameters()) if (j.IsOut) {
							if (j.ParameterType == typeof(object).MakeByRefType())
								_tryDeserialize = new FunctionWrapper(i);
							else if (j.ParameterType == typeof(IEnumerable<object>).MakeByRefType())
								_tryDeserializeRepeatable = new FunctionWrapper(i);
						}
			}

			if (_tryDeserialize == default)
				throw new Exception("static ctor issue: can't find method correlating to TryDeserialize<T>(out T)");

			if (_tryDeserializeRepeatable == default)
				throw new Exception("static ctor issue: can't find method correlating to TryDeserialize<T>(out IEnumerable<T>)");
		}

		private static readonly FunctionWrapper _tryDeserialize = default;

		private static readonly FunctionWrapper _tryDeserializeRepeatable = default;
		...
		public bool TryDeserialize<T>(BaseMessage m, out IEnumerable<T> result) {
			if (m == default) throw new ArgumentNullException(nameof(m));
		...
		public MessageDefinition GetDefinitionFor<T>()
			=> this.GetDefinitionForType(typeof(T));
