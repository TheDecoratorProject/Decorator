	...
	public static class Deserializer {
		...
		public static int DeserializeToEvent<T>(T eventClass, Message msg, params object[] extraParams)
			...
			foreach (var method in ReflectionHelper.GetMethodsWithAttribute<DeserializedHandlerAttribute>(eventClass?.GetType() ?? typeof(T)))
				try {
					if (DeserializeToMethod(eventClass, msg, method, extraParams)) methodInvokes++;
				} catch (Exception ex) {
					exceptions.Add(ex);
				}
		...
		private static bool DeserializeToMethod<T>(T eventClass, Message msg, MethodInfo method, params object[] extraParams)
			where T : class, new() {
			// get the args
			var parameters = method.GetParameters();

			if (parameters?.Length < 1)
				throw new CustomAttributeFormatException($"Invalid [{nameof(DeserializedHandlerAttribute)}] - must have at least one parameter");
			...
			if (CanInvokeIEnumerableMethod(deserializingType, msg)) {
				deserializedType = InvokeGenericDeserialize(nameof(DeserializeToIEnumerable), msg, deserializingType.GenericTypeArguments[0]);

				InvokeMethod(eventClass, method, deserializedType, extraParams);
				return true;
				...
				deserializedType = InvokeGenericDeserialize(nameof(Deserialize), msg, deserializingType);
				InvokeMethod(eventClass, method, deserializedType, extraParams);
				return true;
			}
		...
		/// <param name="items">The end result IEnumerable of Ts</param>
		...
		public static IEnumerable<T> DeserializeToIEnumerable<T>(Message msg)
			...
			if (msg.Args.Length % msgPosLength != 0) throw new Exception("Uneven amount of message params");
		...
		public static T Deserialize<T>(Message msg)
			...
			if (msgAttrib.Type != msg.Type) throw new Exception("The base message types aren't equal."); // return OneLinerFail("The base message types aren't equal.", out failErrMsg);
			...
			if (ReflectionHelper.TryGetAttributeOf<ArgumentLimitAttribute>(typeof(T), out var limit) &&
				...
				throw new ArgumentException($"Surpassed the maximum amount of args bound by the {nameof(ArgumentLimitAttribute)}");
			...
			foreach (var property in ReflectionHelper.GetPropertiesWithAttribute<PositionAttribute>(typeof(T))) {
				...
				if (ReflectionHelper.TryGetAttributeOf<PositionAttribute>(property, out var posAttrib))
					...
					if (posAttrib.Position >= 0 && msg.Args?.Length > posAttrib.Position &&
						property.PropertyType.IsAssignableFrom(msg?.Args?[posAttrib.Position]?.GetType() ?? typeof(T))) {
						...
						if (!ReflectionHelper.TryGetAttributeOf<OptionalAttribute>(property, out var _) ||
							...
							// yell at 'em
							throw new Exception($"Unable to set the value of ({property.Name}) to the value ({msg?.Args[posAttrib.Position]})");
						}
					}
			}
